...
int main(int argc ,char **argv)
{
    ...
    //@Daredliuth:
    float pInicioX, pInicioY, pendiente, d_min, x_min, y_min, x_choque, y_choque, pAnteriorX, pAnteriorY;
    bool choque, navegando;
    int tChoque, movimiento, atorado;
    ...
    while( ros::ok()  )
    {
        ...
        while( params.run )
        {
            ...
            case 8:
                ...
                user_sm(intensity,light_readings, lidar_readings, params.laser_num_sensors,params.laser_value, q_light,q_inputs,&movements,&est_sig ,params.robot_max_advance ,params.robot_turn_angle);
            break;

            case 9:

                flg_result=light_follower(intensity, light_readings,&movements,max_advance,max_turn_angle);
                if(flg_result == 1) set_light_position(.5,.25);
            ...
            //@Daredliuth: Algoritmo Bug1.
            case 12:
                if(flagOnce){
                    est_sig = 0;
                    flagOnce = 0;
                    pInicioX = params.light_x - params.robot_x;
		            pInicioY = params.light_y - params.robot_y;
		            pendiente = pInicioX /pInicioY;
                    d_min = 10000;
                    choque = false;
                    navegando = false;
                    tChoque = 0;
                ...
                Bug1(params.light_x, params.light_y, params.robot_x, params.robot_y, params.robot_theta, &movements, &est_sig, params.robot_max_advance,params.robot_turn_angle, light_readings, lidar_readings, intensity, params.laser_num_sensors, pInicioX, pInicioY, pendiente, &d_min, &x_min, &y_min, &x_choque, &y_choque, &choque, &navegando, &movimiento, pAnteriorX, pAnteriorY, &atorado);
                pAnteriorX = params.robot_x;
                pAnteriorY = params.robot_y;
                ...
                //choque ? printf("Choque: true\n") : printf("Choque: false\n");
                //navegando ? printf("Navegando: true\n") : printf("Navegando: false\n");
                //printf("tChoque %d\n",tChoque);
                printf("Movimiento: %d\n", movimiento);
            break;

            //@Daredliuth: Algoritmo Bug2.
            case 13:
                if(flagOnce){
                    est_sig = 0;
                    flagOnce = 0;
                ...
                Bug2(params.light_x, params.light_y, params.robot_x, params.robot_y, params.robot_theta, &movements, &est_sig, params.robot_max_advance,params.robot_turn_angle, light_readings, lidar_readings, intensity, params.laser_num_sensors, pInicioX, pInicioY, pendiente, &d_min, &x_min, &y_min, &x_choque, &y_choque, &choque, &navegando, &movimiento, pAnteriorX, pAnteriorY, &atorado);
                pAnteriorX = params.robot_x;
                pAnteriorY = params.robot_y;
